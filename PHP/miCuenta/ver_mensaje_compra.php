...
$obtenerDatosMensaje = "SELECT fecha_peticion, mensaje, id_usuario_comprador, id_usuario_vendedor, imei_movil FROM $tabla_historiales WHERE id_mensaje = '$idMensaje'";
...
if ($queryObtenerMensaje->num_rows > 0) {
  ...
  while($row = $queryObtenerMensaje->fetch_assoc()) {
    ...
    $id_vendedor[0] = $row['id_usuario_vendedor'];
    $imeiMovil[0] = $row['imei_movil'];
  }
}

// Ahora cogemos los datos del usuario que nos lo quiere comprar:
$obtenerNombreComprador = "SELECT nombre, email FROM $tabla_usuarios WHERE uid = '$id_comprador[0]'";
...
if ($queryObtenerNombreComprador->num_rows > 0) {
  while($row = $queryObtenerNombreComprador->fetch_assoc()) {
    ...
    $email_comprador[0] = $row['email'];
  }
}

// Ahora cogemos los datos del usuario vendedor:
$obtenerEmailVendedor = "SELECT email, nombre FROM $tabla_usuarios WHERE uid = '$id_vendedor[0]'";
$queryObtenerEmailVendedor = $con->query($obtenerEmailVendedor);
// Controlamos que la consulta tenga resultados
if ($queryObtenerEmailVendedor->num_rows > 0) {
  while($row = $queryObtenerEmailVendedor->fetch_assoc()) {
    $email[0] = $row['email'];
    $vendedor[0] = $row['nombre'];
  }
}
...
// Vemos si el usuario quiere enviar el mensaje
if (isset($_POST['enviarMensaje']) ) {
    $quiereEnviarMensaje = true;
    $mensaje_enviar = $_POST['enviarMensaje'];
    echo "Hola";
}
...
<body>
  ...
  <div class="container-fluid" id="contenedorMensajes">
    ...
    <?php
    if ($quiereEnviarMensaje) {
        ini_set( 'display_errors', 1 );
        error_reporting( E_ALL );
        $from = $email_comprador[0];
        $to = $email[0];
        $subject = "Mensaje de compra de móvil: ".$marca[0]." ".$modelo[0];
        $message = $mensaje_enviar;
        $headers =  'MIME-Version: 1.0' . "\r\n";
        $headers .= 'From: <'.$email_comprador[0].'>' . "\r\n";
        ...
        <div class="form-group row" id="alertaRemovible">
          ...
          <div class="col-10 col-md-8 alert alert-primary alert-dismissible">
            ...
            <strong>¡Mensaje enviado!</strong>Lo más probable es que el vendedor se ponga pronto en contacto con usted.
            Mensaje enviado a la dirección: '; echo $email[0] .'.
            Mensaje enviado desde la dirección: '; echo $email_comprador[0] .'
          </div>
          <div class="col-1 col-md-2"></div>
        </div>
        ';
    ...
    <form enctype="multipart/form-data" id="formularioVerMensaje" name="formularioVerMensaje" action="ver_mensaje_compra.php?idMensaje=<?php echo $idMensaje; ?>" method="post">
    ...
    <div class="col-12 col-lg-6">
        ...
        <div class="form-group row">
            ...
            <div class="col-12 col-md-6 m-auto divCampos" id="divComprador">
              <label for="comprador" class="col-12 col-form-label font-weight-bold">Vendedor:</label>
              <div class="col-12">
                  <input disabled value="
                  <?php
                  echo $vendedor[0];
            ...
            <div class="col-12 m-auto divCampos">
                <label for="contacto" class="col-12 col-form-label font-weight-bold"><?php echo $vendedor[0]; ?> ha aceptado su solicitud de compra. Pongasé en contacto con el/ella a través del siguiente email:</label>
                    ...
                    <div class="col-10 m-auto">
                        ...
                        echo $email[0];
                        ?>
                        " type="text" name="contacto" class="form-control text-center" id="contacto">
                    </div>
            </div>
<!--
            <div class="col-12 m-auto divCampos">
                <label for="enviarMensaje" class="col-12 col-form-label font-weight-bold">Ó enviele un mensaje de correo desde esta misma página:</label>
                <div class="col-12 col-md-11 m-auto">
                  <textarea name="enviarMensaje" class="form-control" id="enviarMensaje" rows="10" cols="40" maxlength="400"
                  placeholder="¡Hola vendedor! Me pongo en contacto con usted para recoger el producto y darle el dinero."></textarea>
                </div>
            </div>
-->
            ...
            <div class="col-12 col-md-8 col-lg-6 divCampos m-auto" id="divVolverAtras">
                <br><br>
                  <button type="button" onclick="regresarAtras()" class="btn-lg btn-outline-secondary btn-block">Volver a mensajes recibidos</button>
            </div>

<!--
            <div class="col-12 col-md-8 col-lg-6 divCampos m-auto" id="divVolverAtras">
                <br><br>
                  <button type="button" onclick="enviarMensajillo()" class="btn-lg btn-outline-dark btn-block">Enviar mensaje</button>
            </div>
-->
          </div>
    </div>
